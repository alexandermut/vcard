...
import { addHistoryItem, deleteHistoryItem, getHistoryItem } from '../utils/db';
        ...
        >
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <div className="flex gap-1 shrink-0">
              {/* Note: item.images might be undefined in list view now, so we check if it exists or if we want to show an icon based on something else? 
                  Actually, we removed images from the object. So item.images is undefined.
                  We can't show the image icon anymore unless we store a flag "hasImages".
                  But for now, let's just remove the icon or keep it if it happens to be there.
              */}
...
export const HistorySidebar: React.FC<HistorySidebarProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'list'>('list'); // Force list view
  const [searchQuery, setSearchQuery] = useState('');
  const [showDuplicateFinder, setShowDuplicateFinder] = useState(false);
  const [sortBy, setSortBy] = useState<'date-desc' | 'date-asc' | 'name-asc' | 'name-desc'>('date-desc');
  const fileInputRef = React.useRef<HTMLInputElement>(null);
  const { token, login } = useGoogleContactsAuth();
        ...
        <div className="p-4 pb-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-lg text-sm focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-slate-200 placeholder-slate-400"
            />
            {/* Clear Button */}
            {searchQuery && (
              <button
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 space-y-3">
            ...
            {isSelectionMode ? (
              <div className="space-y-2">
                ...
                <button
                  onClick={async () => {
                    console.log("Bulk Export clicked", { selected: selectedIds.size, hasToken: !!token });
                    if (!token) {
                      console.log("No token - triggering login");
                      toast.info("Bitte zuerst mit Google verbinden...");
                      login(); // Trigger login flow
                      return;
                    }

                    if (!confirm(`${selectedIds.size} Kontakte zu Google exportieren?`)) {
                      console.log("Cancelled");
                      return;
                    }

                    console.log("Starting export...");
                    const toastId = toast.loading("Exportiere zu Google...");
                    let successCount = 0;
                    let failCount = 0;
                    ...
                    for (const id of selectedIds) {
                      try {
                        // Fetch full item to preserve images
                        const fullItem = await getHistoryItem(id);
                        if (!fullItem) continue;
                        if (fullItem.googleResourceName) continue; // Skip already saved

                        const vcardData = parseVCardString(fullItem.vcard);
                        const googlePerson = mapVCardToGooglePerson(vcardData.data);
                        const result = await createGoogleContact(token, googlePerson);

                        // Update local history item
                        const updatedItem = { ...fullItem, googleResourceName: result.resourceName };
                        await addHistoryItem(updatedItem);

                        // Update local state (lightweight)
                        const index = updatedHistory.findIndex(h => h.id === id);
                        if (index !== -1) {
                          updatedHistory[index] = { ...updatedHistory[index], googleResourceName: result.resourceName };
                        }
                    ...
                    toast.success(`${successCount} Kontakte exportiert (${failCount} Fehler).`);
                    setIsSelectionMode(false);
                    setSelectedIds(new Set());
                  }}
                  disabled={selectedIds.size === 0}
                  className="w-full py-2 px-3 bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-bold transition-colors flex items-center justify-center gap-2 shadow-sm"
                >
                  <div className="text-lg">G</div>
                  {selectedIds.size > 0 ? `${selectedIds.size} zu Google` : 'Zu Google'}
                </button>
                <button
