...
import { X, Send, Bot, User, Trash2, MessageSquare } from 'lucide-react';
...
export const ChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose, history, apiKey, llmConfig, lang }) => {
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
                <div className="bg-white dark:bg-slate-900 w-full max-w-lg h-[600px] rounded-2xl shadow-2xl flex flex-col pointer-events-auto border border-slate-200 dark:border-slate-800">
                    ...
                    <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 rounded-t-2xl">
                        <div className="flex items-center gap-2 font-semibold text-slate-800 dark:text-slate-100">
                            ...
                            <h3>Chat with Contacts</h3>
                        </div>
                        <div className="flex items-center gap-2">
                            <button onClick={clearChat} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full text-slate-500 transition-colors" title="Clear Chat">
                                <Trash2 size={18} />
                            </button>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-50/50 dark:bg-slate-950/50">
                        {messages.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-center p-8">
                                ...
                                <p className="font-medium">Ask questions about your contacts!</p>
                                <p className="text-sm mt-2">"Who works at Google?"</p>
                                <p className="text-sm">"Do I have a contact named Max?"</p>
                            </div>
                        )}
                    ...
                    {/* Input */}
                    <form onSubmit={handleSubmit} className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-b-2xl">
                        <div className="flex gap-2">
                            <input
                                value={input}
                                onChange={(e) => setInput(e.target.value)}
                                placeholder="Ask a question..."
                                className="flex-1 bg-slate-100 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 dark:text-white placeholder-slate-400"
                                disabled={isLoading}
                            />
                            <button
                                type="submit"
